<a href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}&title=Free Dareen Tatour"
   target="_blank">
    <button class="btn btn-primary"><i class="fa fa-facebook fa-2"></i> Share on Facebook</button>
</a>

<a href="https://twitter.com/intent/tweet?text={{urlencode($tweet->tweet . ' ' . url()->current() )}}"
   target="_blank">
    <button class="btn btn-social btn-twitter"><i class="fa fa-twitter"></i>Share on Twitter
    </button>
</a>